...
/* Quantity of food stored on each planet per month*/
var planetInvt {1..month+1, PLANET, FOOD} >= 0;
...
## Planet Inventory stats at 0
subject to planetInvtStart {p in PLANET}:
    sum {f in FOOD} planetInvt[1, p, f] = 0;

## Calculate stantad inventory quantity.
...
## transportation takes LESS than a month. For all proposites its considered instantaneous
subject to transportTime {t in 1..month, p in PLANET, f in FOOD}:
    transportOut[t, p, f] = transportIn[t, p, f];

## Calculate planet invtentory quantity.
## The inventory quantity for the NEXT month is equal to inventory
## and transportation quantity of the PREVIOUS month minus the
## amount of product sold previously
subject to planetInvtQuantity {t in 1..month, p in PLANET, f in FOOD}:
    planetInvt[t+1, p, f] = planetInvt[t, p, f] + transportIn[t, p, f] - sell[t, p, f];
...
maximize profit:
    ...
    - sum {t in 1..month, p in PLANET, f in FOOD} planetInvt[t+1, p, f] * planetInvtCostPerUnit
